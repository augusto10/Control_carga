...
async function testDirectPostgresConnection() {
  console.log('=== TESTE DE CONEXÃO DIRETA COM POSTGRES ===');
  ...
  if (datasourceUrl.startsWith('prisma+postgres://')) {
    ...
    datasourceUrl = datasourceUrl.replace('postgres://', 'postgresql://');
    console.log('🔄 Convertendo postgres:// para postgresql://');
  }

  console.log('🔗 URL final (formato):', datasourceUrl.split('://')[0] + '://***');
  ...
  try {
    console.log('🔄 Testando conexão com PostgreSQL direto...');
...
// Executar o teste
testDirectPostgresConnection().then(success => {
  console.log('\n=== RESUMO DO TESTE ===');
  if (success) {
