...
async function testPrismaAccelerateConnection() {
  console.log('=== TESTE DE CONEXÃO COM PRISMA ACCELERATE ===');
  ...
  // Usar a URL original do Prisma Accelerate sem modificação
  const datasourceUrl = process.env.DATABASE_URL;
  console.log('🔗 Usando URL do Prisma Accelerate (formato):', datasourceUrl.split('://')[0] + '://***');

  // Criar Prisma Client com a URL original
  ...
  try {
    console.log('🔄 Testando conexão com Prisma Accelerate...');
    
    // Teste simples de conexão
    await prisma.$connect();
    console.log('✅ Conexão com Prisma Accelerate estabelecida com sucesso!');
    ...
    if (user) {
      ...
      // Testar se a senha está criptografada (se tiver campo senha)
      ...
      if (userWithPassword) {
        console.log('✅ Campo de senha encontrado (criptografado)');
      }
    ...
    console.error('❌ Erro na conexão com Prisma Accelerate:');
...
// Executar o teste
testPrismaAccelerateConnection().then(success => {
  console.log('\n=== RESUMO DO TESTE ===');
  if (success) {
