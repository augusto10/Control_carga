...
async function syncProducts() {
  try {
    ...
    while (hasMore) {
      console.log(`📥 Buscando produtos (offset: ${offset}, limit: ${limit})...`);
      ...
      if (!Array.isArray(produtos)) {
        // Tentar extrair array de uma propriedade 'data' ou similar
      ...
      console.log(`📥 ${produtos.length} produtos obtidos nesta página (total: ${allProdutos.length})`);
    ...
    console.log(`✅ ${allProdutos.length} produtos obtidos da API`);
    ...
    const catalogProducts = allProdutos.map((produto) => ({
