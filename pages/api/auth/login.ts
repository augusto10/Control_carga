...
const CORS_HEADERS = {
  ...
  'Access-Control-Allow-Headers': 'X-CSRF-Token, X-Requested-With, Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, X-Api-V, Authorization',
...
const allowCors = (fn: (req: NextApiRequest, res: NextApiResponse) => Promise<void>) => 
  async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === 'OPTIONS') {
      console.log('[API] Requisição OPTIONS recebida, retornando 200');
...
const handler = async (
  ...
  const isReserved = reservedDomains.some((reserved: string) => {
    ...
    if (lowerDomain === lowerReserved || 
        lowerDomain.endsWith('.' + lowerReserved)) {
      return true;
    }
  ...
  if (isReserved) {
    const errorMessage = 'Domínio de email não permitido';
    console.log(`[API] Erro: ${errorMessage} - ${domain}`);
    return res.status(400).json({
      success: false,
      message: errorMessage,
  ...
  */
  ...
  // Verificar se há espaços em branco no início ou no final
  if (email !== email.trim()) {
    const errorMessage = 'O e-mail não pode conter espaços em branco no início ou no final';
    console.log(`[API] Erro: ${errorMessage}`);
    return res.status(400).json({
      success: false,
      message: errorMessage,
      code: 'EMAIL_WHITESPACE'
    });
  }
  
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const MAX_EMAIL_LENGTH = 255; // Limite comum para colunas de email
