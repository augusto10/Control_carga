...
async function testDatabaseConnectionFixed() {
  console.log('=== TESTE DE CONEXÃO COM BANCO DE DADOS (CORRIGIDO) ===');
  ...
  // Para desenvolvimento, precisamos converter prisma+postgres:// para postgresql://
  let datasourceUrl = process.env.DATABASE_URL;
  
  if (process.env.NODE_ENV !== 'production') {
    // Em desenvolvimento, converte prisma+postgres:// para postgresql://
    if (datasourceUrl.startsWith('prisma+postgres://')) {
      datasourceUrl = datasourceUrl.replace('prisma+postgres://', 'postgresql://');
      console.log('🔄 Convertendo prisma+postgres:// para postgresql:// para desenvolvimento');
    }
  }
...
// Executar o teste
testDatabaseConnectionFixed().then(success => {
  console.log('\n=== RESUMO DO TESTE ===');
  if (success) {
